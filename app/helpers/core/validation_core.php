<?php
	abstract class Validation_Core
	{
		...
		protected static function setError($var, $value, $msg)
		{
			...
			if (!isset(self::$errors[(string)self::$object][$var]))
			self::$errors[(string)self::$object][$var] = $msg;
		}
		
		public static function isValid($object)
		{
			// Has Any Errors
			return !empty(self::$errors[$object]);
		}
		...
		public static function check()
		{
			...
			self::$validVars = array_keys(get_object_vars(self::$object));
			// ID and Model not Needed
			self::$validVars = array_slice(self::$validVars, 0, -2);
			
			// Init Error Array
			self::$errors[(string)self::$object] = array();
